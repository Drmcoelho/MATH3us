...
12. **Regime visual: o gibi rigoroso (E23)** — diretiva do autor: o capítulo é **conduzido** pela imagem, não ilustrado por ela. A densidade de referência é a de um gibi — muita figura, muita fórmula em destaque, prosa curta entre elas. Quatro regras operacionais:
    - *Imagem como espinha.* Todo objeto novo, todo enunciado central e toda demonstração vêm acompanhados de elemento gráfico — canvas interativo, figura estática (SVG inline) ou diagrama. Capítulo sem nenhum elemento gráfico está em **violação de template**, não em estilo alternativo. O canvas interativo continua sendo observação, nunca prova (a separação objeto/representação/observação permanece intacta); mas a ausência dele empobrece o capítulo tanto quanto a ausência de prova o invalidaria.
    - *Fórmula em vitrine.* Toda fórmula central aparece em **bloco de destaque** (display), nunca diluída na prosa. A notação é abundante e assumida: o tratado prefere a fórmula bem exposta à paráfrase verbal dela. A renderização respeita E4/E14 (representação textual ou MathML no HTML final; LaTeX rasterizado como imagem é vetado) e E9 (autocontenção: nenhuma biblioteca de renderização via CDN).
    - *Prosa que não repete a figura.* A §1.9 permanece vigente — o argumento é prosa contínua — mas o argumento **encurta**: o texto não descreve o que a figura já mostra; o texto justifica o que a figura não pode provar. Figura mostra, prosa demonstra, box comenta.
    - *Dois boxes regulares.* A **Caixa de curiosidade** — fato lateral que ilumina um objeto do capítulo, admitida na exata medida da cláusula final da §1.11: curiosidade estruturalmente ancorada, nunca depósito enciclopédico. E a **Caixa de segredo oculto** — estrutura que o capítulo já contém sem nomear, enunciada como mistério verificável, com a porta e o capítulo em que será revelada declarados (é a forma em box da porta trancada da §6.2; o segredo aponta para diante, nunca antecipa a solução).

    A regra vale retroativamente como pendência editorial dos capítulos integrados, com prioridade máxima ao Capítulo 4, que na data desta emenda não possui nenhum elemento gráfico.
